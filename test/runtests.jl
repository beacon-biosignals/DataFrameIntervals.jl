...
@testset "DataFrameIntervals.jl" begin
    ...
    df_combined = combine(groupby_interval_join(df1, quarters, r"quar|lab", on=:span), :x => mean)
    df_combined = combine(groupby_interval_join(df1, quarters, Cols(:quarter, r"lab"), on=:span), :x => mean)
    df_combined = combine(groupby_interval_join(df1, quarters, Not([:span, :x]), on=:span), :x => mean)
    ...
    @test_throws ErrorException combine(groupby_interval_join(df2, quarters, on=:span, [:i_dont_exist]), :x => mean)
